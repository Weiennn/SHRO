(function () {
  ...
  function saveQuestion(question) {
    ...
    if (loggedQuestions.has(question)) {
      console.log(
    ...
    chrome.storage.local.get(["questions"], function (result) {
      ...
      chrome.storage.local.set({ questions: existingQuestions }, function () {
        if (chrome.runtime.lastError) {
          console.error(
    ...
    if (userMessages.length === 0) {
      ...
      for (const selector of alternativeSelectors) {
        ...
        if (messages.length > 0) {
          console.log(
            `ChatGPT Logger: Found ${messages.length} messages with selector: ${selector}`
          );
    ...
    chrome.storage.local.get(["questions"], function (result) {
      if (chrome.runtime.lastError) {
        ...
        existingQuestions.forEach((q) => {
          loggedQuestions.add(q.question);
        });
        console.log(
          `ChatGPT Logger: Loaded ${loggedQuestions.size} existing questions`
        );
      }
